...
import { fetchSavedItems as fetchSavedItemsAction, removeSavedItem as removeSavedItemAction } from '../store/savedItemsSlice';
...
const SavedItems = () => {
  ...
  useEffect(() => {
    const loadSavedItems = async () => {
      try {
        ...
        const items = await savedItemService.fetchSavedItems(user.userId);
        
        // Update Redux state
        dispatch(fetchSavedItemsAction(items));
      } catch (error) {
  ...
  const removeFromSaved = async (id) => {
    try {
      ...
      // Update Redux state
      dispatch(removeSavedItemAction(id));
